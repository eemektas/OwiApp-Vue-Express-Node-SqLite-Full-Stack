...
export default {
  ...
  watch: {
    '$route.query.search': {
      ...
      handler: async function () {
        ...
        this.resp = (await WeatherService.weather(this.place.city)).data
        console.log('json', JSON.parse(JSON.stringify(this.resp)))
      }
    }
  },
  components: {
  ...
  async mounted () {
    try {
      const cityId = this.$store.state.route.params.cityId
      this.palce.city = (await WeatherService.show(cityId)).data
    } catch (err) {
      console.log(err)
    }
  },
  methods: {
    async sendPlace () {
    const this.resp = await WeatherService.post(this.place)
    }
  }
